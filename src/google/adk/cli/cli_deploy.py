...
def to_agent_engine(
  ...
  try:
    ...
    if otel_to_cloud:
      ...
      if 'OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT' in env_vars:
        click.secho(
            'Ignoring OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT in'
            ' .env as `--otel_to_cloud` was explicitly passed and takes'
            ' precedence',
            fg='yellow',
        )
